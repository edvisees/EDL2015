...
public class WeightedPageRankPowerMethod extends WeightedPageRank {
    private final static Logger LOGGER = LoggerFactory.getLogger( WeightedPageRankPowerMethod.class );
	...
	public void step() throws IOException {
		...
		while( n-- != 0 ) {
			...
			if ( outdegree == 0 || buckets != null && buckets.get( i ) ) accum += oldRank[ i ];
				...
				while ( j-- != 0 ) {
					newRank[ succ[ j ] ] += ( oldRank[ i ] * lab[j].getFloat() );
				}
			}
...
//		this.logger.info("current delta of norm " + norm + "is: " + delta);
		return delta;
	}
